...
如果你的配置 **没有独立显卡**（如 i5-8400 / i5-9400 等仅有 UHD 630 核显的 CPU），需要做以下修改：

### BIOS 设置（重要！）

| 设置项 | 值 |
| --- | --- |
| CPU Core Graphics | Enable（或 AUTO） |
...
1. **删除独显相关驱动**（可选）：
   - 删除 `EFI/OC/Kexts/RadeonSensor.kext`
   - 删除 `EFI/OC/Kexts/SMCRadeonGPU.kext`

2. **添加核显配置 DeviceProperties**（通过 OpenCore Configurator 或 PlistEditPro）：
   - 路径：`DeviceProperties → Add`
   - 添加 PciRoot(0x0)/Pci(0x2,0x0)：
     - `AAPL,ig-platform-id`：填入 `0300923E`（桌面 Coffee Lake UHD 630）
     - `framebuffer-patch-enable`：1
     - `framebuffer-cursormem`：`00000001`
     - `device-id`：`923E0000`

3. **启动参数（boot args）**：
   - 在 `NVRAM → Add → 7C436110-AB2A-4BBB-A880-3B59815779C3 → boot-args` 中添加：
     - `-igfxvesa`（如果显示器黑屏则保留，正常启动后可删除）
     - `agdpmod=pikera`（仅在使用 AMD 独显时需要，核显用户不需要）

4. **显卡设置为仅使用核显**：
   - 在 BIOS 中将「优先显示输出」设置为 CPU 主板接口（HDMI/DP）

### 常见问题

- **显示器黑屏**：先尝试加 `-igfxvesa` 参数，确认能进入系统后再逐步去掉
- **核显无法硬解**：确保 `framebuffer-patch-enable` 设为 1，platform-id 正确
- **睡眠失效**：删除 `HibernationFixup.kext` 后重试
